document.addEventListener("DOMContentLoaded", function() {
    // 비밀번호 확인 함수
    function checkPwd() {
        const pwd = document.querySelector("#tcPwd").value; // 비밀번호 필드
        const pwdCheck = document.querySelector("#checkPwdTea").value; // 비밀번호 확인 필드
...
// 아이디 중복확인
$(function(){
    const idInput = document.querySelector("#enrollForm-form input[name=tcId]");
...
function handleSubmit() {
    ...
    if (teacherRadio.checked) {
        statusInput.value = "N"; // 선생님일 경우
    } else {
        statusInput.value = "A"; // 관리자일 경우
    }
    return true; // 폼 제출 허용
}